...
class NBase(NAbstract):
        ...
        if len(new_names) != len(old_names):
            ...
            if node_type == 'spike':
                for i, name in enumerate(new_names):
                    ...
                    node.set_name(name)
                    self._spike_by_name[name] = self._spike_by_name.pop(old_names[i])

            elif node_type == 'lfp':
                for i, name in enumerate(new_names):
                    node = self.get_lfp(old_names[i])
                    node.set_name(name)
                    self._lfp_by_name[name] = self._spike_by_name.pop(old_names[i])
    ...
    def set_spike_names(self, names):
        ...
        self.change_names(self, self.get_spike_names(), names, 'lfp')
    ...
    def set_lfp_names(self, names):
        ...
        self.change_names(self, self.get_lfp_names(), names, 'lfp')
    ...
    def _add_node(self, cls, node, node_type, **kwargs):
        ...
        new_node = self._new_instance(cls, node, **kwargs)
        self.add_node(new_node, node_type, replace=kwargs.get('replace', False))
